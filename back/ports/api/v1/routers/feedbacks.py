...
async def get_date_feedback(
    ...
    feedbacks = {f["day_time"]: f for f in feedbacks}
    return FeedbackResponse.from_feedbacks(
        morning=feedbacks.get(DayTime.morning),
        evening=feedbacks.get(DayTime.evening),
    )
