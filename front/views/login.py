...
class Login(ft.UserControl, BaseView):
    LOGIN_ENDPOINT: str = f"http://{Config.BASE_URL}/api/v1/login"
    ...
    def on_login_click(self, e: ft.ControlEvent):
        ...
        self.clean()
        response = requests.post(self.LOGIN_ENDPOINT, json=body.dict())
        if response.status_code == 200:
            login_response = LoginResponseDTO(**response.json())
            e.page.client_storage.set("access_token", login_response.result)
